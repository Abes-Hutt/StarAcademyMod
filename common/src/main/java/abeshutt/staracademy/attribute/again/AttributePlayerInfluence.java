...
public class AttributePlayerInfluence implements SpawningInfluence {
    ...
    public float affectBucketWeight(SpawnBucket bucket, float weight) {
        Attribute<Rational> attribute = AttributeHolder.getRoot(this.player)
                .path(Attributes.ofBucketWeight(bucket.getName()));
        Option<Rational> result = attribute.get(Option.present(Rational.of(weight)),
